      ...
      <div className="bg-pink-50 shadow-2xl rounded-2xl p-10 transform hover:scale-105 transition-transform duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {["name", "email", "phone", "address", "message"].map((field) => (
            <div key={field}>
              <label
                className="block text-lg font-semibold mb-2 capitalize"
                htmlFor={field}
              >
                {field}
              </label>
          ...
          <button
            type="submit"
            className="w-full bg-blue-900 hover:bg-blue-700 text-white font-bold py-3 rounded-lg shadow-md transition-colors"
          >
            Send Message
          </button>
        </form>
